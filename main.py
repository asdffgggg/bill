...
async def searchfor_bill(data: SearchQ):
    ...
    if data.number:
        ...
        return main_list(
            laws=None, custom_bills=get_congressional_bills(data.congress, data.chamber)
        )
    ...
    return (
        ...
        Main(
            Div(
                ...
                Div(
                    Iframe(
                        src=f"/bill/pdf/{congress}/{number}/{_type}",
                        width="100%",
                        height="100%",
                    ),
                    Div(
                        Zero_md(
                            Template(), Script(type="text/markdown", id="response")
                        ),
...
async def model_bill_handler(msg: str, send):
    ...
    if CONNECTIONS.get(id, None) is None:
        ...
        CONNECTIONS[id] = {"last_time": time.time(), "messages": []}
    ...
    if phase == 0:
        ...
        response = Script(
            "\n\n---\n\n", id="response", type="text/markdown", hx_swap_oob="beforeend"
        )
